...
# ═══════════════════════════════════════════════════════════
# Nemeth 查表（comp8 子模式用）
# 啟動時從 nemethdefs.cti parse，邊界 0 剝除
# ═══════════════════════════════════════════════════════════
_NEMETH: dict = {}
_NEMETH_MAX_LEN: int = 1

def _build_nemeth_table(cti_path: str) -> None:
    global _NEMETH, _NEMETH_MAX_LEN
    import re as _re

    def _dotstr_to_bits(s):
        bits = 0
        for ch in s:
            if ch.isdigit():
                d = int(ch)
    ...
    table = {}
    try:
        with open(cti_path, encoding='utf-8', errors='replace') as f:
            for line in f:
                line = line.strip()
                if not line or line.startswith('#'):
                    continue
                m = _re.match(
                    r'^(math|sign|digit|letter|punctuation)\s+(\S+)\s+(\S+)',
                    line)
                if not m:
                    continue
                char_tok, dot_tok = m.group(2), m.group(3)
                if _re.match(r'^\\[xX][0-9a-fA-F]+$', char_tok):
                    try: ch = chr(int(char_tok[2:], 16))
                ...
                skip = False
                for p in parts:
                    if p == 'a': cell_bits.append(0); continue
                    b = _dotstr_to_bits(p)
                    if b is None: skip = True; break
                    cell_bits.append(b)
                if skip:
                    continue
                key = _strip_zeros(tuple(cell_bits))
                if not key:
                    continue
                if key not in table:
                    table[key] = ch
    ...
    _NEMETH_EXTRA = {
        ...
        (40, 2):      '>',   # 46-2      大於
        (16, 5):      '<',   # 5-13      小於
        (40, 5):      '=',   # 46-13     等於（覆蓋 κ，κ 改走選單）
    }
    table.update(_NEMETH_EXTRA)

    _NEMETH = table
    _NEMETH_MAX_LEN = max((len(k) for k in table), default=1)
...
class TogglePrefsDialog(wx.Dialog):
    """選擇 JKL+Space 循環中包含哪些點字模式（不含一般輸入法）"""
    ...
    def __init__(self, parent, enabled_modes):
        ...
        for mode in _ALL_MODES_WITH_OFF:
            if mode is None:
                continue   # 一般輸入法不加入循環，由 Ctrl+NVDA+7 控制
    ...
    def _on_ok(self, evt):
        ...
        if len(selected) < 1:
            wx.MessageBox('請至少選擇一個模式。', '提示', wx.OK | wx.ICON_WARNING, self)
            return
...
# 檢查更新
# ═══════════════════════════════════════════════════════════
GITHUB_LATEST_API = "https://api.github.com/repos/hurthuang/6d-IME/releases/latest"
_UPDATE_HEADERS = {
    "User-Agent": (
...
def _check_update_worker(silent=False):
    ...
    for asset in data.get("assets", []):
        if asset.get("name", "").endswith(".nvda-addon"):
            ...
            break
    release_url = data.get("html_url", "https://github.com/hurthuang/6d-IME/releases")
...
def _download_and_install_worker(asset_url):
    ...
    tmp_path = os.path.join(tempfile.gettempdir(), "6dIME-update.nvda-addon")
    try:
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def __init__(self):
        ...
        self._jkl_ready          = False
        self._jkl_toggle_consumed = False
        ...
        # Nemeth 子模式（comp8 下 space+dots134 觸發）
        self._nemeth_sub: bool = False
        self._nemeth_buf: list = []  # List[int]

        # 啟動時 parse nemethdefs.cti
        _build_nemeth_table(os.path.join(plugin_dir, 'nemethdefs.cti'))
        log.info(f"6d-IME: Nemeth table {len(_NEMETH)} patterns, max_len={_NEMETH_MAX_LEN}")

        # 偏好：toggle 循環包含的模式，從 NVDA config 載入
        self._enabled_modes: list = self._load_enabled_modes()
        ...
        # 開機背景自動檢查一次更新，延遲幾秒避免搶在 NVDA 啟動流程前面；有新版才提示，沒有則靜默
    ...
    # ── Config 持久化 ────────────────────────────────────
    _CFG_SECTION = '6dIME'
    _CFG_KEY     = 'enabledModes'
    _DEFAULT_MODES = [m for m in [0, 1, 2, 3, 4] if m is not None]  # 全部點字模式

    @classmethod
    def _ensure_config_section(cls):
        if cls._CFG_SECTION not in config.conf:
            config.conf[cls._CFG_SECTION] = {}

    def _load_enabled_modes(self) -> list:
        try:
            self._ensure_config_section()
            raw = config.conf[self._CFG_SECTION].get(self._CFG_KEY, '')
            if raw:
                modes = [int(x) for x in raw.split(',') if x.strip().lstrip('-').isdigit()]
                modes = [m for m in modes if m in (0, 1, 2, 3, 4)]
                if modes:
                    return modes
        except Exception:
            log.debug('6d-IME: _load_enabled_modes 失敗', exc_info=True)
        return list(self._DEFAULT_MODES)

    def _save_enabled_modes(self):
        try:
            self._ensure_config_section()
            config.conf[self._CFG_SECTION][self._CFG_KEY] = ','.join(str(m) for m in self._enabled_modes)
        except Exception:
            log.debug('6d-IME: _save_enabled_modes 失敗', exc_info=True)
    ...
    def script_toggle(self, gesture):
        """開啟/關閉六點輸入（Ctrl+NVDA+7）
        關閉時切到一般輸入法（None）；
        開啟時切到 _enabled_modes 中第一個點字模式。
        """
        if self._mode is not None:
            ui.message(_MODE_NAMES[None])
            wx.CallLater(150, self._flush_and_set_mode, None)
        else:
            brl_modes = [m for m in self._enabled_modes if m is not None]
            if brl_modes:
                next_mode = brl_modes[0]
                ui.message(_MODE_NAMES[next_mode])
                wx.CallLater(150, self._flush_and_set_mode, next_mode)
            else:
                ui.message('請先在偏好設定中選擇至少一個點字模式')

    script_toggle.category = '六點輸入法'

    def script_toggle_cycle(self, gesture):
        """正向循環切換點字輸入模式（JKL+Space）"""
        brl_modes = [m for m in self._enabled_modes if m is not None]
        if not brl_modes:
            return
        if self._mode not in brl_modes:
            next_mode = brl_modes[0]
        else:
            idx = brl_modes.index(self._mode)
            next_mode = brl_modes[(idx + 1) % len(brl_modes)]
        ui.message(_MODE_NAMES[next_mode])
        wx.CallLater(150, self._flush_and_set_mode, next_mode)

    script_toggle_cycle.category = '六點輸入法'
    ...
    __gestures = {
        'kb:NVDA+control+7':          'toggle',          # 開/關六點輸入
        'kb:NVDA+control+shift+7':    'toggle_reverse',  # 反向循環（保留）
        'kb:NVDA+control+8':          'prefs',
        'kb:NVDA+alt+shift+m':        'nemeth_menu',     # Nemeth 符號選單
        'bk:dots':                    'display_dots',    # 攔截點字顯示器格輸入
    }

    def script_nemeth_menu(self, gesture):
        """Nemeth 補充符號選單（Alt+Shift+NVDA+M）：分類 + Backspace 返回上層"""
        _CATEGORIES = [
            ('四則運算', [
                ('÷', '÷ 除號',    '⠨⠌'),
                ('×', '× 乘號',    '⠈⠡'),
        ...
        # 第八類：全部符號，從 _NEMETH 動態生成，按 key 長度再按 bits 排序
        _all_items = []
        for key, ch in sorted(_NEMETH.items(), key=lambda kv: (len(kv[0]), kv[0])):
            brl = ''.join(chr(0x2800 + b) for b in key)
            name = f'U+{ord(ch):04X} {ch}'
            _all_items.append((ch, name, brl))
        _CATEGORIES.append(('全部符號', _all_items))

        def _show():
            gui.mainFrame.prePopup()

            class NemethDialog(wx.Dialog):
                def __init__(self, parent):
                    super().__init__(parent, title='Nemeth 符號選單',
                    ...
                    self._listbox.Bind(wx.EVT_KEY_DOWN, self._on_key)

                    # 隱形 OK button：接收 Enter 鍵（NVDA 環境唯一可靠做法）
                    self._ok_btn = wx.Button(self, wx.ID_OK, label='')
                    self._ok_btn.SetDefault()
                    self._ok_btn.Hide()
                    self.Bind(wx.EVT_BUTTON, self._on_ok_btn, id=wx.ID_OK)

                    sizer = wx.BoxSizer(wx.VERTICAL)
                    sizer.Add(self._listbox, 1, wx.EXPAND | wx.ALL, 8)
                    self.SetSizer(sizer)
                ...
                def _on_ok_btn(self, event):
                    """Enter 鍵觸發（透過隱形 default button）"""
                    self._commit()

                def _show_cats(self, sel=0):
                    self._level = 'cat'
                    self.SetTitle('Nemeth 符號選單')
                    self._listbox.Set([cat for cat, _ in _CATEGORIES])
                    self._listbox.SetSelection(
                        min(sel, self._listbox.GetCount() - 1))

                def _show_syms(self, cat_idx, sel=0):
                    self._level   = 'sym'
                    self._cat_idx = cat_idx
                    cat_name, items = _CATEGORIES[cat_idx]
                    self.SetTitle(cat_name)
                    labels = [f'{name}\t{brl}' for _, name, brl in items]
                    self._listbox.Set(labels)
                    self._listbox.SetSelection(
                        min(sel, self._listbox.GetCount() - 1))
                ...
                def _on_key(self, event):
                    ...
                    if kc == wx.WXK_SPACE:
                        ...
                        if self._level == 'cat':
                            ...
                            event.Skip()
                    elif kc in (wx.WXK_LEFT, wx.WXK_BACK):
                        if self._level == 'sym':
                            self._show_cats(self._cat_idx)
                        else:
                ...
                def _commit(self):
                    ...
                    if self._level == 'cat':
                        ...
                        _, items = _CATEGORIES[self._cat_idx]
                        self._result = items[idx][0]
                        self.EndModal(wx.ID_OK)

            dlg = NemethDialog(gui.mainFrame)
            if dlg.ShowModal() == wx.ID_OK and dlg._result:
                ch = dlg._result
                queueHandler.queueFunction(
                    queueHandler.eventQueue,
                    _send_unicode_chars, ch)
            dlg.Destroy()
            gui.mainFrame.postPopup()

        wx.CallAfter(_show)

    script_nemeth_menu.category = '六點輸入法'

    def _flush_and_set_mode(self, mode):
        """flush 組字區後切換模式（由 wx.CallLater 延後呼叫）。"""
        self._flush_before_mode_change()
        wx.CallLater(70, self._set_mode_silent, mode)

    def _set_mode_silent(self, mode):
        """切換模式但不播報名稱（名稱已在呼叫前播報）。"""
        old = self._mode
        log.debug(f"6d-IME _set_mode_silent: {old!r} → {mode!r}")
        self._reset_state()
    ...
    def _reset_state(self):
        # 不清物理按鍵狀態（_dots_down/_dots_any/_space_down 由 keyup 管理）
        self._brl_buf       = ''
        self._punct_buf     = []
        self._just_sent     = False
        self._ime_composing = False   # 注音模式：是否已送出注音鍵但尚未確認
        self._eng_buf       = []
        self._eng_buf_text  = ''
    ...
    def _has_modifier():
        ...
        return bool(gas(0x11) & 0x8000 or gas(0x12) & 0x8000 or
                    gas(0x5B) & 0x8000 or gas(0x5C) & 0x8000)
    ...
    def _on_key_down(self, vkCode, scanCode, extended, injected):
        ...
        if scanCode == SCAN_SPACE:
            if self._mode is not None:
                ...
                if self._dots_down == (1 << 3 | 1 << 4 | 1 << 5):
                    self._jkl_ready = True
    ...
    def _on_key_up(self, vkCode, scanCode, extended, injected):
        ...
        if scanCode == SCAN_SPACE:
            ...
            if was_down:
                if self._jkl_ready and self._dots_down == (1 << 3 | 1 << 4 | 1 << 5):
                    # JKL 全按且 space 已抬起 → 觸發 toggle（JKL 仍按著）
                    self._jkl_ready = False
                    self._jkl_toggle_consumed = True
                    queueHandler.queueFunction(
                        queueHandler.eventQueue,
                        self.script_toggle_cycle, None)
                elif (self._mode == MODE_COMP8
                      and self._dots_snapshot == 13):   # space+dots134 → Nemeth 子模式
                    self._dots_snapshot = 0
                    queueHandler.queueFunction(
                        queueHandler.eventQueue,
                        self._nemeth_sub_enter)
                elif not self._dots_any:
                    self._process_space()
            return False
        # 點7(A)
        if scanCode == SCAN_A:
        ...
        if scanCode in _BRL_KEY_SCAN:
            ...
            if self._dots_any and self._dots_down == 0:
                ...
                self._dots_snapshot = 0
                self._jkl_ready = False
                if self._jkl_toggle_consumed:
                    self._jkl_toggle_consumed = False   # 消耗：不送格
                else:
                    self._process_cell(bits)
            return False
        return self._old_ku(vkCode, scanCode, extended, injected)
    ...
    def _process_cell(self, bits: int):
        ...
        if self._mode in _ENG_MODES:
            if bits > 255:
                return
            # Nemeth 子模式：累積格，空白鍵 flush
            if self._nemeth_sub:
                self._nemeth_buf.append(bits)
                return
    ...
    def _nemeth_sub_enter(self):
        """進入 Nemeth 子模式：先 flush 目前 eng_buf，再報「數學」。"""
        if self._eng_buf:
            self._eng_translate(False)
        self._nemeth_sub = True
        self._nemeth_buf = []
        ui.message('數學')

    def _nemeth_flush(self):
        """
        空白鍵確認：貪婪最長匹配把 _nemeth_buf 翻譯成符號送出。
        字母格（NABCC 有對應字母）查不到 Nemeth 時直接送字母。
        送出後退出子模式，語音報「電腦點字」。
        """
        buf = list(self._nemeth_buf)
        self._nemeth_buf = []
        self._nemeth_sub = False
        ...
        while i < len(buf):
            matched = False
            # 貪婪最長匹配 Nemeth 表
            for length in range(min(_NEMETH_MAX_LEN, len(buf) - i), 0, -1):
                key = tuple(buf[i:i + length])
                ch = _NEMETH.get(key)
                if ch is not None:
                    out.append(ch)
                    i += length
